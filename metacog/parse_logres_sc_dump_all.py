...
def parse_secondchance_log(log_content, output_file, model_list):
    ...
    # Updated block start regex to capture model name (no correct/incorrect status)
    block_start_regex = re.compile(
        r"--- Analyzing (\S+) \(Redacted, \d+ game files\) ---"
    )
...
def plot_results(df_results, subject_order=None, dataset_name="GPQA_SecondChance"):
    ...
    # Create figure
    fig, axs = plt.subplots(nrows, ncols, figsize=(max(10 * ncols, len(subject_order) * 1.5 + 2 if subject_order else 15), 6 * nrows))
    ...
    if df.empty:
        for col_idx in range(ncols):
            axs[col_idx].text(0.5, 0.5, 'No data', 
    ...
    bars = axs[0].bar(formatted_subject_names, df["AnswerChange%"],
    ...
    axs[0].set_ylabel('Answer Change %', fontsize=label_fontsize)
    axs[0].set_title(f'Answer Change Rate (95% CI)', fontsize=title_fontsize)
    axs[0].axhline(0, color='black', linestyle='--', linewidth=0.8)
    axs[0].axhline(0.25, color='red', linestyle=':', linewidth=0.8, label='25%')
    axs[0].legend(fontsize=legend_fontsize)
    axs[0].tick_params(axis='x', rotation=45, labelsize=tick_fontsize)
    axs[0].tick_params(axis='y', labelsize=tick_fontsize)
    axs[0].set_ylim(0, max(0.7, df["AnswerChange_CI_High"].max() * 1.1))
    ...
    if has_entropy:
        if has_cap_entropy:
            ...
            bars = axs[1].bar(formatted_cap_subjects, cap_values,
                               color='cornflowerblue',
                               yerr=[yerr_cap_low, yerr_cap_high], ecolor='gray', capsize=5, width=0.6)
            
            axs[1].set_ylabel('Coefficient Value', fontsize=label_fontsize)
            axs[1].set_title(f'Capabilities Entropy (Model 4.6 )', fontsize=title_fontsize)
        ...
        if has_norm_prob_entropy:
            ...
            bars = axs[2].bar(formatted_subject_names, norm_values,
                           color='darkorange',
                           yerr=[yerr_norm_low, yerr_norm_high], ecolor='gray', capsize=5, width=0.6)
            
            axs[2].set_ylabel('Coefficient Value', fontsize=label_fontsize)
            axs[2].set_title(f'Normalized Prob Entropy (Model 4.6 )', fontsize=title_fontsize)
    ...
    plt.savefig(f"secondchance_analysis_charts_{dataset_name}_all.png", dpi=300)
    print(f"Charts saved to secondchance_analysis_charts_{dataset_name}_all.png")
