...
if __name__ == "__main__":
    ...
    for model_name_part, game_file_paths_list_for_model in model_game_files.items():
        ...
        if files_feedback_false_or_none:
            feedback_groups_to_process.append(("Feedback_False_Or_None", files_feedback_false_or_none))
        ...
        for feedback_type_str, files_for_current_feedback_type in feedback_groups_to_process:
            ...
            for redaction_type_str, files_for_current_redaction_type in redaction_groups_to_process:
                ...
                for subj_acc_type_str, current_game_files_for_analysis in subj_acc_override_groups_to_process:
                    print(f"      Processing for {subj_acc_type_str} ({len(current_game_files_for_analysis)} files)")
                    ...
                    if not os.path.exists(capabilities_file_path):
                        print(f"        Corresponding capabilities file not found: {capabilities_file_path}. Skipping model for this feedback/redaction/subj_acc group.")
                        continue
                    ...
                    try:
                        ...
                        print(f"        Error loading capabilities file {capabilities_file_path}: {e}. Skipping model for this feedback/redaction/subj_acc group.")
                        continue
                    
                    if not s_i_map_for_this_model:
                        print(f"        No S_i data loaded from {capabilities_file_path}. Skipping model for this feedback/redaction/subj_acc group.")
                        continue

                    if not current_game_files_for_analysis:
                        print(f"        No game files found for model {model_name_part}, feedback {feedback_type_str}, redaction {redaction_type_str}, subj_acc {subj_acc_type_str}. Skipping this group.")
                        continue
                    
                    # Pass the filtered list of game files for the current model, feedback, redaction, and subj_acc type
                    df_model = prepare_regression_data_for_model(current_game_files_for_analysis,
                                                                                        sqa_feature_lookup,
                                                                                        s_i_map_for_this_model)

                    if df_model is None or df_model.empty:
                        print(f"        No data for regression analysis for model {model_name_part}, feedback {feedback_type_str}, redaction {redaction_type_str}, subj_acc {subj_acc_type_str}.")
                        continue
                    ...
                    log_output(f"\n--- Analyzing Model: {model_name_part} ({feedback_type_str}, {redaction_type_str}, {subj_acc_type_str}, {len(current_game_files_for_analysis)} game files) ---", print_to_console=True)
                    ...
                    if current_game_files_for_analysis:
                        ...
                        log_output(f"        No game files found to extract log metrics for {model_name_part}, {feedback_type_str}, {redaction_type_str}, {subj_acc_type_str}.")
                    ...
                    try:
                        ...
                        print(f"        Error during logistic regression for model {model_name_part}, feedback {feedback_type_str}, redaction {redaction_type_str}, subj_acc {subj_acc_type_str}: {e}")
                    
                    print("-" * 40) # This should be per subj_acc_override group